...
## Cross-branch data point (caspar branch, for the joint report)

A second, independent VO run from the caspar branch (own implementation):
**457 points** (frames 0–6840, stride 15), **24.18 m shape RMSE**,
**0.09 m** over a local 10-second window. Note it is **similarity-aligned**
(free scale), so it asserts shape but NOT dimensions — our rigid no-scale
27.6 m is the dimensions-true headline. Two independent pipelines reaching the
same order of magnitude (24–28 m full-flight shape error) is the cross-check.

## Figures (docs/figures/)

Generated by `scripts/09` and `scripts/10` from the committed result files:
| figure | what it shows |
|---|---|
| `trajectory_report.png` | est vs GPS on the orthophoto + SE(2)-aligned shape panel |
| `drift_curve.png` | error vs hops-from-anchor (the VO honesty plot) |
| `error_vs_frame.png` | per-frame error along the flight, coloured by hops |
| `bench_bars.png` | SIFT vs LoFTR per-frame error on the 3-frame bench |
| `coverage.png` | standalone matching (~6%) vs VO-anchored coverage (100%) |
